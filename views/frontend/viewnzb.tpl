...
{if $tvinfo && $release.tvinfoID > 0 && $tvinfo.cover==1}<img class="shadow img-polaroid pull-right" style="margin-right:50px; margin-top:80px;" src="{$smarty.const.WWW_TOP}/covers/tv/{$tvinfo.ID}.jpg" width="180" alt="{$tvinfo.releasetitle|escape:"htmlall"}" />{/if}
{if ($movie && !($release.tvinfoID > 0)) && $movie.cover == 1}<img class="shadow img-polaroid pull-right" style="margin-right:50px; margin-top:80px;" src="{$smarty.const.WWW_TOP}/covers/movies/{$movie.imdbID}-cover.jpg" width="180" alt="{$movie.title|escape:"htmlall"}" />{/if}
...
{if $book && $book.cover == 1}<img class="shadow img-polaroid pull-right" style="margin-right:50px; margin-top:80px;" src="{$smarty.const.WWW_TOP}/covers/book/{$book.ID}.jpg" width="160" alt="{$book.title|escape:"htmlall"}" />{/if}
    ...
    {if $tvinfo && $release.tvinfoID > 0}
		...
		{if $tvinfo.description != ""}
			<dt>Descrition</dt>
			<dd><span class="descinitial">{$tvinfo.description|escape:"htmlall"|nl2br|magicurl|truncate:"350":"</span><a class=\"descmore\" href=\"#\"> more...</a>"}{if $tvinfo.description|strlen > 350}<span class="descfull">{$tvinfo.description|escape:"htmlall"|nl2br|magicurl}</span>{else}</span>{/if}</dd>
		{/if}
		
		{if $tvinfo.genre != ""}
			<dt>Genre</dt>
			<dd>{$tvinfo.genre|escape:"htmlall"|replace:"|":", "}</dd>
		{/if}
		...
		{/if}
			...
			<div style="margin-left:180px; margin-bottom:5px;">
				<a class="label" title="View all episodes from this series" href="{$smarty.const.WWW_TOP}/series/{$release.tvinfoID}">All Episodes</a>
				...
				<a class="label" href="{$smarty.const.WWW_TOP}/rss?tvinfo={$release.tvinfoID}&amp;dl=1&amp;i={$userdata.ID}&amp;r={$userdata.rsstoken}" title="Rss feed for this series">Rss Feed</a>
			</div>
	{/if}

{if $movie && !($release.tvinfoID > 0)}
	<dl class="dl-horizontal" style="margin-right:300px;">
		<dt>Movie Info</dt>
