...
<table class="data highlight Sortable table table-striped" id="browsetable">
	...
	{foreach from=$shows item=show}
		<tr class="{cycle values=",alt"}">
			<td>
				<a title="View details" href="{$smarty.const.WWW_TOP}/series/{$show.tvinfoID}{if $show.categoryID != ''}?t={$show.categoryID|replace:"|":","}{/if}"><h5>{$show.releasetitle|escape:"htmlall"|wordwrap:75:"\n":true}</h5></a>
			</td>
			...
			<td class="mid" style="width:100px;">
				<div class="btn-group">
					<a class="btn btn-mini btn-warning" href="{$smarty.const.WWW_TOP}/myshows/edit/{$show.tvinfoID}" class="myshows" rel="edit" name="series{$show.tvinfoID}" title="Edit Categories">Edit</a>&nbsp;&nbsp;
					<a class="btn btn-mini btn-danger" href="{$smarty.const.WWW_TOP}/myshows/delete/{$show.tvinfoID}" class="myshows" rel="remove" name="series{$show.tvinfoID}" title="Remove from My Shows">Remove</a>
				</div>
				</td>
		</tr>
	{/foreach}
