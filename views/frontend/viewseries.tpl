...
{else}
    ...
    <h2>
        {$tvinforow.releasetitle} {if $catname != ''} in {$catname|escape:"htmlall"}{/if}
    </h2>
    ...
    <div>
        ...
        <div class="btn-group">
            ...
            {if $myshows.ID != ''}
                <a class="btn btn-mini btn-warning" href="{$smarty.const.WWW_TOP}/myshows/edit/{$tvinforow.ID}?from={$smarty.server.REQUEST_URI|escape:"url"}" class="myshows" rel="edit" name="series{$tvinforow.ID}" title="Edit Categories for this show">Edit</a> |
                <a class="btn btn-mini btn-danger" href="{$smarty.const.WWW_TOP}/myshows/delete/{$tvinforow.ID}?from={$smarty.server.REQUEST_URI|escape:"url"}" class="myshows" rel="remove" name="series{$tvinforow.ID}" title="Remove from My Shows">Remove</a>
            {else}
                <a class="btn btn-mini btn-success" href="{$smarty.const.WWW_TOP}/myshows/add/{$tvinforow.ID}?from={$smarty.server.REQUEST_URI|escape:"url"}" class="myshows" rel="add" name="series{$tvinforow.ID}" title="Add to My Shows">Add</a>
            {/if}
        </div>
    </div>

    <div class="tvseriesheading">
        {if $tvinforow.covers == 1}
            <center>
                <img class="shadow img img-polaroid" style="max-height:300px;" alt="{$tvinforow.releasetitle} Logo" src="{$smarty.const.WWW_TOP}/covers/tv/{$tvinforow.ID}.jpg" />
            </center>
            <br/>
        {/if}
        <p>
            {if $tvinforow.genre != ''}<b>{$tvinforow.genre}</b><br />{/if}
            <span class="descinitial">{$tvinforow.description|escape:"htmlall"|nl2br|magicurl}</span>
        </p>
    ...
    <center>
        <div class="btn-group">
            ...
            {if $isadmin}<a class="btn btn-small" href="{$smarty.const.WWW_TOP}/admin/tvinfo-edit.php?id={$tvinforow.ID}&amp;action=update&amp;from={$smarty.server.REQUEST_URI|escape:"url"}">Update</a>{/if}
            <a class="btn btn-small" href="{$smarty.const.WWW_TOP}/rss?tvinfo={$tvinforow.ID}{if $category != ''}&amp;t={$category}{/if}&amp;dl=1&amp;i={$userdata.ID}&amp;r={$userdata.rsstoken}">Rss Feed for this Series <i class="fa-icon-rss"></i></a>
                            ...
                            {foreach $season as $episodes}
                                {foreach $episodes as $result}
                                    <tr class="{cycle values=",alt"}" id="guid{$result.guid}">
                                        ...
                                        <td class="less"><a title="This series in {$result.category_name}" href="{$smarty.const.WWW_TOP}/series/{$result.tvinfoID}?t={$result.categoryID}">{$result.category_name}</a></td>
