...
<form id="nzb_multi_operations_form" action="get">
	...
	<table style="100%" class="data highlight icons table table-striped" id="browsetable">
		...
		<tr class="{cycle values=",alt"}{if $lastvisit|strtotime<$result.adddate|strtotime} new{/if}" id="guid{$result.guid}">
			...
			<td class="item">
				...
				<div class="resextra">
					<div class="btns">{strip}
						...
						{if $result.tvinfoID > 0}
						<a class="badge badge-inverse halffade" href="{$smarty.const.WWW_TOP}/series/{$result.tvinfoID}" title="View all episodes">View Series</a>
						{/if}
			...
			<td class="less">
    <a title="Browse {$result.category_name}" href="{$smarty.const.WWW_TOP}/browse?t={$result.categoryID}">{$result.category_name}</a><br />
    <span style="float: left; margin-top: 5px;"><a title="Browse {$result.group_name}" href="{$smarty.const.WWW_TOP}/browse?g={$result.group_name}">{$result.group_name|replace:"alt.binaries":"a.b"}</a></span>
</td>
