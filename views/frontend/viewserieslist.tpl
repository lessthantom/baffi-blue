	...
	{foreach $serieslist as $sletter => $series}
		...
		{foreach $series as $s}
			<tr class="{cycle values=",alt"}">
				<td><a class="title" title="View series" href="{$smarty.const.WWW_TOP}/series/{$s.ID}">{$s.releasetitle|escape:"htmlall"}</a>{if $s.prevdate != ''}<br /><span class="label">Last: {$s.previnfo|escape:"htmlall"} aired {$s.prevdate|date_format}</span>{/if}</td>
				...
				<td class="mid">
					...
					{if $s.ID > 0}
						<a title="RSS Feed for {$s.releasetitle|escape:"htmlall"}" href="{$smarty.const.WWW_TOP}/rss?tvinfo={$s.ID}&amp;dl=1&amp;i={$userdata.ID}&amp;r={$userdata.rsstoken}"><i class="fa-icon-rss"></i></a>
					{/if}
				</td>
			</tr>
		{/foreach}
